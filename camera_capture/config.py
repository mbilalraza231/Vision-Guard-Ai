...
class CameraConfig(BaseModel):
    ...
    def validate_rtsp_url(cls, v):
        ...
        if not v.startswith(('rtsp://', 'rtmp://', 'http://', 'https://', '/', './')):
            raise ValueError(
                'URL must start with rtsp://, rtmp://, http://, https://, or be a valid file path')
        return v
...
class RedisConfig(BaseModel):
    ...
    retry_on_timeout: bool = Field(
        default=True, description="Retry on timeout")
...
class RetryConfig(BaseModel):
    ...
    max_retries: int = Field(
        default=5, ge=1, description="Maximum retry attempts")
...
class CaptureConfig(BaseModel):
    ...
    buffer: BufferConfig = Field(
        default_factory=BufferConfig, description="Buffer configuration")
